...
- `get_progress_summary`, `update_learner_signal`, branch target metadata, alternate prompt variants, and full MCP resources are not implemented yet.
...
- Progress events are append-only and currently support recent-event listing through the repository only. Events tied to canonical lifecycle mutations must stay in the same repository transaction as that mutation. No MCP progress summary tool exists yet.
...
- Latest known local verification: `90` tests passed with `93.68%` coverage.
