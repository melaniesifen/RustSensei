...
- Next-step adaptive rules beyond the returned assessment action, progress events, `get_progress_summary`, `update_learner_signal`, and full MCP resources are not implemented yet.
...
- Latest known local verification: `73` tests passed with `93.48%` coverage.
