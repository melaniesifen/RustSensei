...
- Alternate prompt variants and full MCP resources are not implemented yet.
...
1. Add alternate prompt variants and deterministic variant rotation.
2. Add MCP resources for progress summary and curriculum concepts.
3. Resolve MCP SDK integration and test tool schemas/calls.
...
- Latest known local verification: `108` tests passed with `93.93%` coverage.
