...
- Progress events, `get_progress_summary`, `update_learner_signal`, branch target metadata, alternate prompt variants, full assignment lifecycle controls, and full MCP resources are not implemented yet.
...
- `get_next_lesson` creates an active assignment, reuses active assignments, returns pending assessment after an attempt, and creates a post-assessment assignment from `repeat`, `simplify`, `continue`, `accelerate`, or branch-fallback actions.
...
- Latest known local verification: `80` tests passed with `93.65%` coverage.
