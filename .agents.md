...
- MCP handler registration is unit-tested through `register_handlers` with a fake registrar. The MCP boundary is not integration-tested against the real SDK because the `mcp` package is not available from the current local package index. The current tool signatures use a `payload` wrapper until FastMCP schema behavior is verified.
...
2. Clean up packaging and setup docs.
...
- `rust_sensei.mcp_server.register_handlers` owns tool, resource, and prompt registration so the MCP surface can be tested without importing the unavailable SDK. `run` remains the only function that imports `mcp.server.fastmcp.FastMCP`.
...
- The base package depends on Pydantic and keeps the official MCP SDK in the optional `mcp` and `dev-mcp` extras because some local package indexes cannot provide `mcp>=1.0.0`. Use `.[dev]` for unit-test work that does not start the real MCP server.
...
- Latest known local verification: `130` tests passed with `93.30%` coverage.
