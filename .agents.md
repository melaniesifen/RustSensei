...
- Agents should run tests through `.venv/bin/python -m pytest`; do not assume `pytest` is on the shell `PATH` or that the virtual environment is activated.
- Latest known local verification: `166` tests passed under Python `3.14.5` in `.venv`; prior coverage passed with `93.30%`.
