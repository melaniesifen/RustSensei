...
- Branch target metadata, alternate prompt variants, and full MCP resources are not implemented yet.
...
1. Add branch target metadata and branch lesson resolution.
2. Add alternate prompt variants and deterministic variant rotation.
3. Add MCP resources for progress summary and curriculum concepts.
4. Resolve MCP SDK integration and test tool schemas/calls.
...
- Latest known local verification: `105` tests passed with `94.10%` coverage.
