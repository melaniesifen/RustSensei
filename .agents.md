...
- Existing code covers package metadata, DTO/domain models, JSON learner profile storage, lesson assignment storage, curriculum seed loading, attempt storage, assessment storage, progress event storage, session service, lesson service, assessment service, progress service, setup service, CLI entrypoint, and tests.
- Implemented flows: `start_session`, `get_learner_profile`, `get_next_lesson`, `submit_attempt`, `assess_attempt`, `get_progress_summary`, and `get_setup_status`.
...
- `update_learner_signal`, branch target metadata, alternate prompt variants, and full MCP resources are not implemented yet.
...
1. Add `update_learner_signal`.
2. Add branch target metadata and branch lesson resolution.
...
5. Resolve MCP SDK integration and test tool schemas/calls.
...
- Latest known local verification: `101` tests passed with `93.99%` coverage.
